...
import { simpleCountState } from "../state/simpleCountState";
...
export default class extends HTMLElement {
  ...
  connectedCallback() {
    // set default value
    const [count] = simpleCountState.get();
    this.count = count;
  }

  disconnectedCallback() {
    // save current value
    const [_count, setCount] = simpleCountState.get();
    setCount(this.count);
  }
  ...
  render() {
    ...
    return html`
      ...
      <p class="mt-2 mb-2">
        Shows how to use @property and @attribute to trigger rerendering on
        changes
      </p>
      ...
      <div class="flex flex-col">
        ...
        <span
          >ele-one - using property decorator - manual rerender delayed</span
        >
        <ele-3 .myvalue=${this.count}></ele-3>

        <span
          >ele-two- using attribute decorator - manual rerender delayed</span
        >
